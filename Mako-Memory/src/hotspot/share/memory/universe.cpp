...
jint Universe::initialize_heap() {
  ...
  // Modified by Haoran
	if( SemeruEnableMemPool && (_collectedHeap->kind() == CollectedHeap::Shenandoah) ){
		log_info(heap)("%s, Using Shenandoah GC, build the Semeru Memory pool. \n", __func__);

		// Create the ShenandoahSemeruCollectedHeap policy and heap space.

		// Semeru
		// 1) [!!]Build the Semeru Colloctor Policy  : universe->_SemeruCollectedHeap
		log_info(heap)("%s, Build the ShenandoahSemeruCollectorPolicy. \n",__func__);
