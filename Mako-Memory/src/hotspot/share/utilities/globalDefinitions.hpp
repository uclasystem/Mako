...
#define CUR_MEMORY_SERVER_ID (size_t)0
// #define CUR_MEMORY_SERVER_ID (size_t)1
...
#define MEMORY_SERVER_0_REGION_START_ID		2
...
#define MEMORY_SERVER_1_REGION_START_ID		18		//debug, single server
#endif
#ifdef SMALL
// #define MEMORY_SERVER_1_REGION_START_ID		9		//debug, single server
#define MEMORY_SERVER_1_REGION_START_ID		10		//debug, single server
#endif

static const char cur_mem_server_ip[]    = "10.0.10.6";
// static const char cur_mem_server_ip[]    = "10.0.10.10";
// static const char cur_mem_server_ip[]    = "10.0.10.11";
...
#ifdef LARGE
#define RDMA_DATA_REGION_NUM   16
#endif
#ifdef SMALL
...
// #define SEMERU_START_ADDR   ((size_t)0x800000000000)
...
#define RDMA_STRUCTURE_SPACE_SIZE  ((size_t) ONE_GB *8)
...
#define KLASS_INSTANCE_OFFSET               (size_t)(ALIVE_BITMAP_OFFSET + ALIVE_BITMAP_SIZE) //       0x400,040,000,000
#define KLASS_INSTANCE_OFFSET_SIZE_LIMIT    (size_t)(256*ONE_MB)                                 



#define MEM_REF_Q_OFFSET                      (size_t)(KLASS_INSTANCE_OFFSET + KLASS_INSTANCE_OFFSET_SIZE_LIMIT)// (SATB_BUFFER_OFFSET + SATB_BUFFER_SIZE_LIMIT + 512 * (size_t)ONE_MB) //       0x400,050,000,000
#define MEM_REF_Q_LEN                         (size_t)0 // (size_t)(8387584) // (size_t)(67107840)       //(256M - 4K) * 1024 / 4
#define MEM_REF_Q_SIZE_LIMIT                  (size_t)0 // (size_t)(64 * (size_t)ONE_MB) // (size_t)(512 * (size_t)ONE_MB)
#define MEM_REF_Q_PER_SIZE                    (size_t)0 // (size_t)(32 * (size_t)ONE_MB) // (size_t)(256 * (size_t)ONE_MB)
...
//     The structure of SemeruHeapRegion. 4K for each Region is enough.
//     [x] precommit all the space.
#define HEAP_REGION_MANAGER_OFFSET           (size_t)(KLASS_INSTANCE_OFFSET + KLASS_INSTANCE_OFFSET_SIZE_LIMIT) // (KLASS_INSTANCE_OFFSET + KLASS_INSTANCE_OFFSET_SIZE_LIMIT)  // +1G+256MB,  0x400,050,000,000
#define HEAP_REGION_MANAGER_SIZE_LIMIT       (size_t)(0*ONE_MB) // each SemeruHeapRegion should less than 4K, this is enough for 65536 HeapRegion.


// 3.1.1 CPU Server To Memory server, Initialization
// [x] precommit
#define CPU_TO_MEMORY_INIT_OFFSET     (size_t)(HEAP_REGION_MANAGER_OFFSET + HEAP_REGION_MANAGER_SIZE_LIMIT) // +1GB +256MB 0x400,050,200,000
#define CPU_TO_MEMORY_INIT_SIZE_LIMIT (size_t) 2 * ONE_MB // 256*ONE_MB    //

// 3.1.2 CPU Server To Memory server, GC
// [x] precommit
#define CPU_TO_MEMORY_GC_OFFSET       (size_t)(CPU_TO_MEMORY_INIT_OFFSET + CPU_TO_MEMORY_INIT_SIZE_LIMIT) // +1GB +258MB 0x400,050,400,000
#define CPU_TO_MEMORY_GC_SIZE_LIMIT   (size_t) 0 * ONE_MB // 256*ONE_MB    //


// 3.1.3 Memory server To CPU server 
// [x] precommit
#define MEMORY_TO_CPU_GC_OFFSET       (size_t)(CPU_TO_MEMORY_GC_OFFSET + CPU_TO_MEMORY_GC_SIZE_LIMIT) // +1GB +258MB 0x400,050,400,000
#define MEMORY_TO_CPU_GC_SIZE_LIMIT   (size_t) 2 * ONE_MB // 256*ONE_MB    //


// 3.1.4 Synchonize between CPU server and memory server
// [x] precommit
#define SYNC_MEMORY_AND_CPU_OFFSET       (size_t)(MEMORY_TO_CPU_GC_OFFSET + MEMORY_TO_CPU_GC_SIZE_LIMIT) // +1GB +260MB 0x400,050,600,000
#define SYNC_MEMORY_AND_CPU_SIZE_LIMIT   (size_t) 4 * ONE_MB // 256*ONE_MB    //
...
// 5.1 Memory server CSet
// [x] precommit
#define MEMORY_SERVER_CSET_OFFSET     (size_t)(SYNC_MEMORY_AND_CPU_OFFSET + SYNC_MEMORY_AND_CPU_SIZE_LIMIT)    // +1GB +262MB 0x400,050,800,000
#define MEMORY_SERVER_CSET_SIZE       (size_t)PAGE_SIZE   // 4KB 

// 5.2 cpu server state, STW or Mutator 
// Used as CPU <--> Memory server state exchange
// [x] precommit
#define FLAGS_OF_CPU_SERVER_STATE_OFFSET    (size_t)(MEMORY_SERVER_CSET_OFFSET + MEMORY_SERVER_CSET_SIZE)  // +1GB +262MB +4KB 0x400,050,801,000
#define FLAGS_OF_CPU_SERVER_STATE_SIZE      (size_t)PAGE_SIZE      // 4KB 


// 5.3 memory server flags 
// [x] precommit
#define FLAGS_OF_MEM_SERVER_STATE_OFFSET    (size_t)(FLAGS_OF_CPU_SERVER_STATE_OFFSET + FLAGS_OF_CPU_SERVER_STATE_SIZE)  // +1GB +262MB +8KB 0x400,050,802,000
#define FLAGS_OF_MEM_SERVER_STATE_SIZE      (size_t)PAGE_SIZE      // 4KB 
...
// Reserve 4KB for the write check flags.
// [x] precommit
#define FLAGS_OF_CPU_WRITE_CHECK_OFFSET       (size_t)(FLAGS_OF_MEM_SERVER_STATE_OFFSET + FLAGS_OF_MEM_SERVER_STATE_SIZE)  // +1GB +262MB +12KB 0x400,050,803,000
#define FLAGS_OF_CPU_WRITE_CHECK_SIZE_LIMIT   (size_t)PAGE_SIZE     // 4KB 

#define SATB_METADATA_MEM_SERVER_OFFSET           (size_t)(FLAGS_OF_CPU_WRITE_CHECK_OFFSET+FLAGS_OF_CPU_WRITE_CHECK_SIZE_LIMIT)  // +1GB +262MB +16KB 0x400,050,804,000
#define SATB_METADATA_MEM_SERVER_SIZE             (size_t)PAGE_SIZE     // 4KB 
#define SATB_METADATA_CPU_SERVER_OFFSET           (size_t)(SATB_METADATA_MEM_SERVER_OFFSET+ SATB_METADATA_MEM_SERVER_SIZE)  // +1GB +262MB +20KB 0x400,050,805,000
#define SATB_METADATA_CPU_SERVER_SIZE             (size_t)PAGE_SIZE     // 4KB 


#define INDIRECTION_METADATA_OFFSET           (size_t)(SATB_METADATA_CPU_SERVER_OFFSET+ SATB_METADATA_CPU_SERVER_SIZE)  // +1GB +262MB +24KB 0x400,050,806,000
#define INDIRECTION_METADATA_SIZE             (size_t)PAGE_SIZE     // 4KB 


#define RDMA_FLAG_OFFSET                          (size_t)(INDIRECTION_METADATA_OFFSET + INDIRECTION_METADATA_SIZE) // +1GB +262MB +28KB 0x400,050,807,000
...
#define CSET_SYNC_MAP_OFFSET                   (size_t)(RDMA_FLAG_OFFSET + RDMA_FLAG_SIZE)    // +1GB +263MB 0x400,050,900,000
#define CSET_SYNC_MAP_SIZE                     (size_t)(1 * (size_t)ONE_MB)


#define COMPACTED_MAP_OFFSET                   (size_t)(CSET_SYNC_MAP_OFFSET + CSET_SYNC_MAP_SIZE)    // +1GB +264MB 0x400,050,a00,000
...
// [?] Not sure how much space is needed for the cross-region-reference queue, give all the rest space to it. Need to shrink it latter.
//
#define CROSS_REGION_REF_UPDATE_Q_OFFSET      (size_t)(COMPACTED_MAP_OFFSET + COMPACTED_MAP_SIZE)    // +1GB +265MB 0x400,050,b00,000
#define CROSS_REGION_REF_UPDATE_Q_SIZE_LIMIT  (size_t)(16 * (size_t)ONE_MB)
#define CROSS_REGION_REF_UPDATE_PADDING       (size_t)(16 * (size_t)ONE_MB)
#define CROSS_REGION_REF_UPDATE_Q_LEN         (size_t)(130000) // 1M * 1024/8



#define SATB_BUFFER_OFFSET                    (size_t)(CROSS_REGION_REF_UPDATE_Q_OFFSET + CROSS_REGION_REF_UPDATE_PADDING) // +1GB +266MB 0x400,050,c00,000
#define SATB_BUFFER_SIZE_LIMIT                (size_t)(16 * (size_t)ONE_MB) //(/*256*/1024 * (size_t)ONE_MB)


// +512MB for bitmap

// #define OFFSET_TABLE_OFFSET                    (size_t)(SATB_BUFFER_OFFSET + SATB_BUFFER_SIZE_LIMIT) // +1GB +282MB 0x400,051,c00,000



// ----------------------------------Indirection Table Related MetaData----------------------------------

// #define OFFSET_TABLE_START_ADDR (size_t)(SEMERU_START_ADDR + OFFSET_TABLE_OFFSET)
// #define MAX_HEAP_SIZE  (size_t)(32 * ONE_GB)
// #define OFFSET_TABLE_MAX_SIZE (size_t)(MAX_HEAP_SIZE / 8)

#define ALIVE_TABLE_PAGE_SIZE (size_t)(512) //(4096/8)
#define LOG_ALIVE_TABLE_PAGE_SIZE (size_t)(9) // 2^9=512
#define ALIVE_TABLE_OFFSET (size_t)(SATB_BUFFER_OFFSET + SATB_BUFFER_SIZE_LIMIT)
#define ALIVE_TABLE_START_ADDR (size_t)(SEMERU_START_ADDR + ALIVE_TABLE_OFFSET)
#define MAX_HEAP_SIZE  (size_t)(32 * ONE_GB)
#define ALIVE_TABLE_MAX_SIZE (size_t)(MAX_HEAP_SIZE / ALIVE_TABLE_PAGE_SIZE * 4)
...
// #define RDMA_RESERVE_PADDING_OFFSET (size_t)(OFFSET_TABLE_OFFSET + OFFSET_TABLE_MAX_SIZE)
#define RDMA_RESERVE_PADDING_OFFSET (size_t)(ALIVE_TABLE_OFFSET + ALIVE_TABLE_MAX_SIZE)
...
// #define MAX_FREE_MEM_GB   ((size_t) REGION_SIZE_GB * RDMA_DATA_REGION_NUM + RDMA_STRUCTURE_SPACE_SIZE/ONE_GB + 4)    //for local memory management
// #define MAX_REGION_NUM    ((size_t) MAX_FREE_MEM_GB/REGION_SIZE_GB + 1)     //for msg passing, ?
// #define MAX_SWAP_MEM_GB   (u64)(REGION_SIZE_GB * RDMA_DATA_REGION_NUM + 4)		// Space managed by SWAP
