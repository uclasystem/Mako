...
void* Build_rdma_to_cpu_server(void* _args ){
  ...
  const char* ip_str    = cur_mem_server_ip;
	...
	// heap_size		=	(size_t)args->heap_size + 0x100000000;
...
void init_memory_pool(char* heap_start, size_t heap_size, struct context* rdma_ctx ){
  ...
  for(i=1;i<rdma_ctx->mem_pool->region_num + 1;i++){
...
void handle_cqe(struct ibv_wc *wc){
  ...
  if (wc->status != IBV_WC_SUCCESS) {
    if(wc->status == IBV_WC_WR_FLUSH_ERR){
        tty->print("wc->status: %d\n", wc->status);
        tty->print("handle_cqe: status is not IBV_WC_SUCCESS.");
    }
    else {
      tty->print("wc->status: %d\n", wc->status);
      die("handle_cqe: status is not IBV_WC_SUCCESS or IBV_WC_WR_FLUSH_ERR");
    }
  ...
  if(rdma_session->connected == 0){
   ...
   //   SemeruRDMA_lock->wait(Mutex::_no_safepoint_check_flag);
    ...
    //      Or we will get BAD_ADDRESS error.
	  for(i=0; i< rdma_session->mem_pool->region_num + 1; i++){
    // for(i=0; i< rdma_session->mem_pool->region_num + 1; i++){
...
void send_free_mem_size(struct semeru_rdma_queue * rdma_queue){
  ...
  // rdma_session->send_msg->mapped_chunk = rdma_session->mem_pool->region_num + 1; // 1 meta data Region, N data Region
...
void send_regions(struct semeru_rdma_queue * rdma_queue){
  ...
  // rdma_session->send_msg->mapped_chunk = rdma_session->mem_pool->region_num + 1; 
