...
public:
   ...
   */
  // Modified by Haoran
  inline bool semeru_mark_other(oop obj);
  inline bool mark(oop obj);

  inline bool is_marked(oop obj) const;

  // Modified by Haoran2
  bool allocated_after_mark_start(HeapWord* addr) const;

  inline MarkBitMap* mark_bit_map();

  HeapWord* top_at_mark_start(ShenandoahHeapRegion* r) const;
  // Modified by Haoran
  HeapWord* semeru_top_at_mark_start(ShenandoahSemeruHeapRegion* r) const;
  ...
  // Modified by Haoran
  void semeru_capture_top_at_mark_start(ShenandoahSemeruHeapRegion* r);
  void semeru_reset_top_at_mark_start(ShenandoahSemeruHeapRegion* r);
  void semeru_initialize_top_at_mark_start(ShenandoahSemeruHeapRegion* r);

  void reset_top_bitmap(ShenandoahHeapRegion *r);
  void clear_bitmap(ShenandoahHeapRegion *r);

  // Modified by Haoran
  void semeru_reset_top_bitmap(ShenandoahSemeruHeapRegion *r);
  void semeru_clear_bitmap(ShenandoahSemeruHeapRegion *r);
...
public:
  ShenandoahAliveTable(ShenandoahSemeruHeap* heap, HeapWord* heap_start, size_t heap_size_in_words, size_t region_size_in_words, size_t page_size_in_words, size_t log_page_size_in_words, uint* table_start) :
  _heap(heap),
