...
// Modified by Haoran
ShenandoahSemeruMemoryPool::ShenandoahSemeruMemoryPool(ShenandoahSemeruHeap* heap) :
        CollectedMemoryPool("ShenandoahSemeru",
                            heap->initial_capacity(),
                            heap->max_capacity(),
                            true /* support_usage_threshold */),
                            _heap(heap) {}

MemoryUsage ShenandoahSemeruMemoryPool::get_memory_usage() {
  size_t initial   = initial_size();
  size_t max       = max_size();
  size_t used      = used_in_bytes();
  size_t committed = _heap->committed();
