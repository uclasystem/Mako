...
#ifndef SHARE_VM_GC_SHENANDOAH_SEMERU_SHENANDOAHSCHEDULERTHREAD_HPP
#define SHARE_VM_GC_SHENANDOAH_SEMERU_SHENANDOAHSCHEDULERTHREAD_HPP
...
class ShenandoahSemeruPeriodicTask : public PeriodicTask {
private:
  ShenandoahSemeruControlThread* _thread;
public:
  ShenandoahSemeruPeriodicTask(ShenandoahSemeruControlThread* thread) :
          PeriodicTask(100), _thread(thread) {}
  virtual void task();
};

// Periodic task to flush SATB buffers periodically.
class ShenandoahSemeruPeriodicSATBFlushTask : public PeriodicTask {
public:
  ShenandoahSemeruPeriodicSATBFlushTask() : PeriodicTask(ShenandoahSATBBufferFlushInterval) {} // Haoran: TODO  SemeruSATBBuffer
  virtual void task();
};

class ShenandoahSemeruControlThread: public ConcurrentGCThread {
...
private:
  ...
  ShenandoahSemeruPeriodicTask _periodic_task;
  ShenandoahSemeruPeriodicSATBFlushTask _periodic_satb_flush_task;
...
private:
  ...
  ShenandoahSemeruHeap::ShenandoahDegenPoint _degen_point;

  DEFINE_PAD_MINUS_SIZE(0, DEFAULT_CACHE_LINE_SIZE, sizeof(volatile size_t));
  volatile size_t _allocs_seen;
  DEFINE_PAD_MINUS_SIZE(1, DEFAULT_CACHE_LINE_SIZE, 0);

  bool check_cancellation_or_degen(ShenandoahSemeruHeap::ShenandoahDegenPoint point);
  void service_concurrent_normal_cycle(GCCause::Cause cause);
  void service_stw_full_cycle(GCCause::Cause cause);
  void service_stw_degenerated_cycle(GCCause::Cause cause, ShenandoahSemeruHeap::ShenandoahDegenPoint point);
