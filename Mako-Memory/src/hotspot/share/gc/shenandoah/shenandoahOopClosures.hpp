...
// Modified by Haoran
class ShenandoahSemeruMarkRefsSuperClosure : public MetadataVisitingOopIterateClosure {
private:
  ShenandoahObjToScanQueue* _queue;
...
public:
  ShenandoahSemeruMarkRefsSuperClosure(ShenandoahObjToScanQueue* q, ReferenceProcessor* rp);
};
...
// Modified by Haoran
class ShenandoahSemeruMarkRefsClosure : public ShenandoahSemeruMarkRefsSuperClosure {
private:
  template <class T>
  inline void do_oop_work(T* p)     { work<T, NONE, NO_DEDUP>(p); }

public:
  ShenandoahSemeruMarkRefsClosure(ShenandoahObjToScanQueue* q, ReferenceProcessor* rp) :
    ShenandoahSemeruMarkRefsSuperClosure(q, rp) {};
...
// class ShenandoahSemeruUpdateHeapRefsClosure : public OopIterateClosure {
// private:
//   ShenandoahSemeruHeap* _heap;
...
// public:
//   ShenandoahSemeruUpdateHeapRefsClosure(ShenandoahSemeruHeap* heap) :
//     _heap(heap) {}
...
//   virtual bool do_metadata() { ShouldNotCallThis(); return false; }
//   virtual void do_klass(Klass* k) {ShouldNotCallThis();}
//   virtual void do_cld(ClassLoaderData* cld) {ShouldNotCallThis();}
// };
class ShenandoahSemeruUpdateHeapRefsClosure : public BasicOopIterateClosure {
private:
  ShenandoahSemeruHeap* _heap;
