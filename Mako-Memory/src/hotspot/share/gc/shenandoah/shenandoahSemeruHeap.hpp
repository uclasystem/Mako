...
#ifndef SHARE_VM_GC_SHENANDOAH_SEMERU_SHENANDOAHHEAP_HPP
#define SHARE_VM_GC_SHENANDOAH_SEMERU_SHENANDOAHHEAP_HPP
...
class ShenandoahSemeruCollectorPolicy;
class ShenandoahSemeruControlThread;
class ShenandoahGCSession;
class ShenandoahHeuristics;
class ShenandoahMarkingContext;
class ShenandoahAliveTable;
class ShenandoahPhaseTimings;
class ShenandoahSemeruHeap;
class ShenandoahSemeruHeapRegion;
class ShenandoahHeapRegionClosure;
class ShenandoahSemeruCollectionSet;
class ShenandoahSemeruFreeSet;
class ShenandoahSemeruConcurrentMark;
...
class VMStructs;
// class RootObjectQueue;
// class received_memory_server_cset;
// class flags_of_cpu_server_state;
// class flags_of_mem_server_state;
// class flags_of_rdma_write_check;
// class SATBMetadataMemServer;
// class SATBMetadataCPUServer;

class ShenandoahSemeruRegionIterator : public StackObj {
private:
  ...
  ShenandoahSemeruRegionIterator(const ShenandoahSemeruRegionIterator& that);
  ShenandoahSemeruRegionIterator& operator=(const ShenandoahSemeruRegionIterator& o);

public:
  ShenandoahSemeruRegionIterator();
  ShenandoahSemeruRegionIterator(ShenandoahSemeruHeap* heap);
  ...
  inline ShenandoahSemeruHeapRegion* next();
...
class ShenandoahSemeruHeapRegionClosure : public StackObj {
public:
  virtual void heap_region_do(ShenandoahSemeruHeapRegion* r) = 0;
  virtual bool is_thread_safe() { return false; }
};

#ifdef ASSERT
class ShenandoahSemeruAssertToSpaceClosure : public OopClosure {
private:
  template <class T>
...
//
class ShenandoahSemeruHeap : public CollectedHeap {
...
//
public:
  static ShenandoahSemeruHeap* heap();
  static ShenandoahSemeruHeap* heap_no_check();

  const char* name()          const { return "Shenandoah"; }
  ShenandoahSemeruHeap::Name kind() const { return CollectedHeap::Shenandoah; }

  ShenandoahSemeruHeap(ShenandoahSemeruCollectorPolicy* policy);
  jint initialize();
...
private:
  ...
  size_t    _num_regions;
  ShenandoahSemeruHeapRegion** _regions;
  ShenandoahSemeruRegionIterator _update_refs_iterator;

public:
  inline size_t num_regions() const { return _num_regions; }
  inline bool is_heap_region_special() { return _heap_region_special; }

  inline ShenandoahSemeruHeapRegion* const heap_region_containing(const void* addr) const;
  inline size_t heap_region_index_containing(const void* addr) const;

  inline ShenandoahSemeruHeapRegion* const get_region(size_t region_idx) const {
  ...
  // inline ShenandoahSemeruHeapRegion* const get_corr_region(size_t region_idx) const {
  ...
  void heap_region_iterate(ShenandoahSemeruHeapRegionClosure* blk) const;
  void parallel_heap_region_iterate(ShenandoahSemeruHeapRegionClosure* blk) const;



// ---------- Semeru Reserve Memory
...
public:
  ...
  RootObjectQueue* root_object_queue() { return _root_object_queue; }
  size_t  get_server_id(HeapWord* addr);
...
public:
  ...
  SATBMetadataMemServer* _satb_metadata_mem_server;
  SATBMetadataCPUServer* _satb_metadata_cpu_server;

  MemRefQueue*  _mem_ref_queues[NUM_OF_MEMORY_SERVER];

  RDMAByteFlag* _mem_ref_current_queue_has_data;
  RDMAByteFlag* _mem_ref_current_queue_all_here;
  RDMAByteFlag* _mem_ref_other_queue_has_data;
  RDMAByteFlag* _tracing_current_cycle_processing;

  RDMAByteFlag** _assure_read_flags;
...
private:
  ...
  RootObjectQueue* _root_object_queue;
  ShenandoahSemeruControlThread*   _control_thread;
  ShenandoahSemeruCollectorPolicy* _shenandoah_policy;
  // ShenandoahSemeruCollectorPolicy* _shenandoah_semeru_policy;
  ShenandoahHeuristics*      _heuristics;
  ShenandoahSemeruFreeSet*         _free_set;

  // Haoran: TODO
  ShenandoahSemeruConcurrentMark*  _scm;
...
// Haoran: TODO
  ShenandoahSemeruControlThread*   control_thread()          { return _control_thread;    }
  ShenandoahMarkCompact*     full_gc()                 { return _full_gc;           }

public:
  ShenandoahSemeruCollectorPolicy* shenandoah_policy() const { return _shenandoah_policy; }
  ShenandoahHeuristics*      heuristics()        const { return _heuristics;        }
  ShenandoahSemeruFreeSet*         free_set()          const { return _free_set;          }

  // Haoran: TODO
  ShenandoahSemeruConcurrentMark*  concurrent_mark()         { return _scm;               }
...
public:
  ...
  ShenandoahSemeruCollectorPolicy* semeru_collector_policy() const;
...
//
private:
  HeapWord* allocate_memory_under_lock(ShenandoahSemeruAllocRequest& request, bool& in_new_region);
...
public:
  HeapWord* allocate_memory(ShenandoahSemeruAllocRequest& request);
  ...
  // Modified by Haoran for concurrency
  void flush_cache_for_region(ShenandoahSemeruHeapRegion *r);
  void flush_cache_bitmap();
  void flush_cache_range(size_t start, size_t end);

  void calc_offset(ShenandoahSemeruHeapRegion* region);
...
public:
  ...
  inline ShenandoahAliveTable* alive_table() const {
    return _alive_table;
  }

  template<class T>
  inline void marked_object_iterate(ShenandoahSemeruHeapRegion* region, T* cl);

  template<class T>
  inline void marked_object_iterate(ShenandoahSemeruHeapRegion* region, T* cl, HeapWord* limit);

  template<class T>
  inline void marked_object_oop_iterate(ShenandoahSemeruHeapRegion* region, T* cl, HeapWord* limit);

  void reset_mark_bitmap();
  void switch_offset_table(ShenandoahSemeruHeapRegion* region, ShenandoahSemeruHeapRegion* to_region);
  ...
  bool commit_bitmap_slice(ShenandoahSemeruHeapRegion *r);
  bool uncommit_bitmap_slice(ShenandoahSemeruHeapRegion *r);
  bool is_bitmap_slice_committed(ShenandoahSemeruHeapRegion* r, bool skip_self = false);
...
//
private:
  ShenandoahSemeruCollectionSet* _collection_set;
...
public:
  ...
  ShenandoahSemeruCollectionSet* collection_set() const { return _collection_set; }

  template <class T>
  inline bool in_update_set(T obj) const;
  ...
  inline void update_object(oop obj);
...
public:
  ...
  template <class T>
  inline void semeru_maybe_update_with_forwarded(T* p);
  ...
  void trash_humongous_region_at(ShenandoahSemeruHeapRegion *r);
