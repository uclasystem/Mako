...
// Modified by Haoran
template<class T, UpdateRefsMode UPDATE_REFS, StringDedupMode STRING_DEDUP>
inline void ShenandoahSemeruMarkRefsSuperClosure::work(T *p) {
  // Modified by Haoran
  // ShenandoahSemeruConcurrentMark::mark_through_entry<T, UPDATE_REFS, STRING_DEDUP>(p, _heap, _queue, _mark_context);
  ShenandoahSemeruConcurrentMark::mark_through_ref<T, UPDATE_REFS, STRING_DEDUP>(p, _heap, _queue, _mark_context);
}
