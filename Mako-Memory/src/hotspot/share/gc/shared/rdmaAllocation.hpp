  ...
  */
  ALWAYSINLINE void* operator new(size_t instance_size, size_t index ) throw() {
    ...
    switch(Alloc_type)  // based on the instantiation of Template
    {
      case CPU_TO_MEM_AT_INIT_ALLOCTYPE :
        ...
        // commit_size = align_up(instance_size, ObjectAlignmentInBytes);
        commit_size = align_up(instance_size, 16);
  ...
  ALWAYSINLINE void* operator new(size_t instance_size, size_t element_length, size_t index ) throw() {
    ...
    case CROSS_REGION_REF_UPDATE_QUEUE_ALLOCTYPE :
        ...
        tty->print("0x%lx, 0x%lx, 0x%lx, 0x%lx", (size_t)requested_addr, (size_t)commit_size, SEMERU_START_ADDR + CROSS_REGION_REF_UPDATE_Q_OFFSET, CROSS_REGION_REF_UPDATE_Q_SIZE_LIMIT);
    ...
    case MEM_REF_QUEUE_ALLOCTYPE :
        
        assert(false, "Not implement.");
        
        requested_addr = (char*)(SEMERU_START_ADDR + MEM_REF_Q_OFFSET + index * commit_size) ;

        tty->print("0x%lx, 0x%lx, 0x%lx, 0x%lx", (size_t)requested_addr, (size_t)commit_size, SEMERU_START_ADDR + MEM_REF_Q_OFFSET, MEM_REF_Q_SIZE_LIMIT);
        assert(requested_addr + commit_size <= (char*)(SEMERU_START_ADDR + MEM_REF_Q_OFFSET + MEM_REF_Q_SIZE_LIMIT), 
                                                        "%s, Exceed the TARGET_OBJ_QUEUE's space range. \n", __func__ );
        ...
        // [?] How to handle the failure ?
        //     The _alloc_ptr is useless here.
        old_val = CHeapRDMAObj<E, MEM_REF_QUEUE_ALLOCTYPE>::_alloc_ptr;
        ret = Atomic::cmpxchg(requested_addr + commit_size, &(CHeapRDMAObj<E, MEM_REF_QUEUE_ALLOCTYPE>::_alloc_ptr), old_val);
        assert(ret == old_val, "%s, Not MT Safe. \n",__func__);
