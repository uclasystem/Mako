...
// Modified by Haoran
BufferNode* BufferNode::semeru_allocate(size_t size, volatile size_t* index) {
  size_t byte_size = (((size * sizeof(void*) + buffer_offset())-1)/PAGE_SIZE + 1)*PAGE_SIZE  ;
  size_t old_index = *index;
  size_t ret = Atomic::cmpxchg(old_index + 1, index, old_index);
  assert(ret == old_index, "%s, Not MT Safe. \n",__func__);

  char* requested_addr = (char*)(SEMERU_START_ADDR + SATB_BUFFER_OFFSET + byte_size * old_index);
  if(byte_size * old_index + byte_size >= SATB_BUFFER_SIZE_LIMIT) {
	  ShouldNotReachHere();
  }
  // log_debug(semeru, alloc)("Allocate [0x%lx, 0x%lx). Buffer Allocator %lu", (size_t)requested_addr, (size_t)byte_size, old_index);

  os::commit_memory_or_exit(requested_addr, byte_size, !ExecMem, "Buffer Allocator (commit)");  // Commit the space.
  void* data = (void*) requested_addr;
  // void* data = NEW_C_HEAP_ARRAY(char, buffer_offset() + byte_size, mtGC);
  return new (requested_addr) BufferNode;
}
