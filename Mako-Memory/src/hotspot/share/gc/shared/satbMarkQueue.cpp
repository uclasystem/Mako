...
#include "gc/shenandoah/shenandoahSemeruHeap.hpp" // Modified by Haoran
...
// Modified by Haoran
// Haoran: TODO need atomicity
// Haoran: TODO could optimize
bool SATBMarkQueueSet::semeru_apply_closure_to_completed_buffer(SATBBufferClosure* cl) {
  BufferNode* nd = NULL;
  {
    // Modified by Haoran2
    MutexLocker x(_cbl_mon, Mutex::_no_safepoint_check_flag);
    // Have processed node from the first to _completed_buffers_head(included)
    if (_completed_buffers_head == NULL) { // Haoran: TODO need to set it to NULL after each tracing cycle
      if(ShenandoahSemeruHeap::heap()->_satb_metadata_cpu_server->_cpu_server_completed_buffers_tail != 0) {
        _completed_buffers_head = (BufferNode* )ShenandoahSemeruHeap::heap()->_satb_metadata_cpu_server->_cpu_server_completed_buffers_head;
        nd = _completed_buffers_head;
        ShenandoahSemeruHeap::heap()->_satb_metadata_mem_server->_mem_server_completed_buffers_head = (size_t) _completed_buffers_head;
      }
    }
    else {
      if(_completed_buffers_head->next() != NULL) {
        _completed_buffers_head = _completed_buffers_head->next();
        nd = _completed_buffers_head;
        ShenandoahSemeruHeap::heap()->_satb_metadata_mem_server->_mem_server_completed_buffers_head = (size_t) _completed_buffers_head;
      }
    ...
    // if (_completed_buffers_head == NULL) { // Haoran: TODO need to set it to NULL after each tracing cycle
    //   if(ShenandoahSemeruHeap::heap()->_satb_metadata_cpu_server->_cpu_server_completed_buffers_tail != NULL) {
    //     _completed_buffers_head = (BufferNode* )ShenandoahSemeruHeap::heap()->_satb_metadata_cpu_server->_cpu_server_completed_buffers_head;
    //   }
    // }
    // if(_completed_buffers_head->next() != NULL) {
