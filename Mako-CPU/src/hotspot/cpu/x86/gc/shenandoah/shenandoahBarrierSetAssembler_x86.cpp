...
void ShenandoahBarrierSetAssembler::arraycopy_prologue(MacroAssembler* masm, DecoratorSet decorators, BasicType type,
  ...
  {
    Register thread = NOT_LP64(rax) LP64_ONLY(r15_thread);
    assert_different_registers(dst, src, thread); // we don't care about src here?
    // Avoid runtime call when not marking.
    Label done2;
    Address gc_state2(thread, in_bytes(ShenandoahThreadLocalData::gc_state_offset()));
    __ testb(gc_state2, ShenandoahHeap::EVACUATION);
    __ jcc(Assembler::zero, done2);
    ...
    if (UseCompressedOops) {
      ...
      __ call_VM_leaf(CAST_FROM_FN_PTR(address, ShenandoahRuntime::write_ref_array_pre_oop_entry_evac), 2);
    }
#endif
    __ popa();
    __ bind(done2);
