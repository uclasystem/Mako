...
void PtrQueue::enqueue_completed_buffer() {
  ...
  // if(SemeruEnableMemPool && (Universe::heap()->kind() == CollectedHeap::Shenandoah)) {
  //   // Haoran: TODO budget
  //   // Haoran: debug
  //   qset()->enqueue_completed_buffer(node);
  //   // qset()->semeru_enqueue_completed_buffer(node);
  // }
  // else {
  //   qset()->enqueue_completed_buffer(node);
  // }
...
BufferNode* PtrQueueSet::get_completed_buffer(size_t stop_at) {
  ...
  // tty->print("_n_completed_buffers: %lu\n", _n_completed_buffers);
  assert(_n_completed_buffers > 0, "invariant");
  // tty->print("_completed_buffers_head: %lu\n", (size_t)_completed_buffers_head);
  assert(_completed_buffers_head != NULL, "invariant");
  // tty->print("_completed_buffers_tail: %lu\n", (size_t)_completed_buffers_tail);
  ...
  if (_completed_buffers_head == NULL) {
    tty->print("NULL: _n_completed_buffers: %lu\n", _n_completed_buffers);
    assert(_n_completed_buffers == 0, "invariant");
    _completed_buffers_tail = NULL;
    _process_completed_buffers = false;
  }
