...
bool ShenandoahFreeSet::touches_bounds(size_t num) const {
  ...
  // return num == _collector_leftmost || num == _collector_rightmost || num == _mutator_leftmost || num == _mutator_leftmost + _max/2 ||num == _mutator_rightmost;
...
void ShenandoahFreeSet::adjust_bounds() {
  ...
  // // Haoran: _leftmost starts with any in two with free status; rightmost ends with exactly current region with free status
  // while (_mutator_leftmost < _max/2 && !is_mutator_free(_mutator_leftmost) && !is_mutator_free(_mutator_leftmost + _max/2)) {
  //   _mutator_leftmost++;
  // }
  // while (_mutator_rightmost > 0 && !is_mutator_free(_mutator_rightmost)) {
  //   _mutator_rightmost--;
...
HeapWord* ShenandoahFreeSet::allocate_contiguous(ShenandoahAllocRequest& req) {
  ...
  while (true) {
    ...
    // if ( !is_mutator_free(end) || !is_empty_or_trash(_heap->get_region(end)) ||  !is_empty_or_trash(_heap->get_region(end + _max/2)))  {
      end++;
      beg = end;
      continue;
    }
...
void ShenandoahFreeSet::rebuild() {
  ...
  for (size_t idx = 0; idx < _heap->num_regions(); idx++) {
    ...
    if (region->is_alloc_allowed() || region->is_trash()) {
      ...
      if (_heap->is_evacuation_in_progress() && _heap->collection_set()->is_in_update_set(idx)) continue;
...
//  // Modified by Haoran for evacuation
//   // for (size_t idx = 0; idx < _heap->num_regions(); idx++) {
//   size_t middle_num = _heap->num_regions()/2;
  
//   for (size_t idx = 0; idx < middle_num; idx++) {
//     ShenandoahHeapRegion* region = _heap->get_region(idx);
    
//     if (region->is_alloc_allowed() || region->is_trash()) {
//     // if (region->is_alloc_allowed() || region->is_trash() || region->is_trash_partial()) {
//       assert(!region->is_cset(), "Shouldn't be adding those to the free set");
//       ShenandoahHeapRegion* corr_region = _heap->get_region(idx + middle_num);
//       if (corr_region->is_empty() || corr_region->is_trash()) {
//         // Do not add regions that would surely fail allocation
//         if (has_no_alloc_capacity(region)) continue;
//         _capacity += alloc_capacity(region);
//         assert(_used <= _capacity, "must not use more than we have");
...
//         assert(!is_mutator_free(idx + middle_num), "We should not add it");
//         assert(!is_collector_free(idx + middle_num), "We are about to add it");
//         assert(is_empty_or_trash(_heap->get_region(idx + middle_num)), "The corresponding region should be empty or trash");
//       }
//       else if(corr_region->is_cset()) {
//         if(corr_region->is_pinned() || region->is_pinned()) {
//           ShouldNotReachHere();
//         }
//         // assert(_heap->in_update_set(region->bottom()), "This region should be in update set.");

//         assert(!region->is_cset(), "Shouldn't be adding those to the free set");
//         assert(region->is_trash() || region->is_empty(), "Corr_region should not be trash or something else");
//         assert(!is_mutator_free(idx), "We should not add it");
//         assert(!is_collector_free(idx), "We are about to add it");
//         _collector_free_bitmap.set_bit(idx);
//       }
//       else {
//         assert(corr_region->is_regular() || corr_region->is_pinned(), "Corr_region should be regular/pinned region");
//         assert(region->is_empty() || region->is_trash(), "One of them must be empty");
//         assert(!region->is_cset(), "Shouldn't be adding those to the free set");
//         assert(!is_mutator_free(idx), "We are not to add it, it shouldn't be there");
//         assert(!corr_region->is_cset(), "Shouldn't be adding those to the free set");
//         assert(!is_mutator_free(idx), "We are about to add it, it shouldn't be there already");
      
//         // Do not add regions that would surely fail allocation
//         if (has_no_alloc_capacity(corr_region)) continue;
//         _capacity += alloc_capacity(corr_region);
//         assert(_used <= _capacity, "must not use more than we have");

//         _mutator_free_bitmap.set_bit(idx+middle_num);
//       }
//     }
...
//       assert(!corr_region->is_cset(), "Shouldn't be adding those to the free set");
//       assert(corr_region->is_trash() || corr_region->is_empty(), "Corr_region should not be trash or something else");
//       assert(!is_mutator_free(idx + middle_num), "We should not add it");
//       assert(!is_collector_free(idx + middle_num), "We are about to add it");
//       _collector_free_bitmap.set_bit(idx + middle_num);
//     }
//     else {
//       assert(region->is_humongous(), "Must be this!");
//     }
//   }
