...
void ShenandoahRuntime::write_ref_array_pre_oop_entry_evac(oop* src, oop* dst) {
  ShenandoahBarrierSet *bs = ShenandoahBarrierSet::barrier_set();
  if(src!=NULL)
    bs->load_reference_barrier_array_copy(src);
  if(dst!=NULL)
    bs->load_reference_barrier_array_copy(dst);
}
