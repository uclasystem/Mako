...
ShenandoahControlThread::ShenandoahControlThread() :
  ...
  _allocs_seen(0),
  _first(true) // Modified by Haoran
{
...
void ShenandoahControlThread::service_concurrent_normal_cycle(GCCause::Cause cause) {
  ...
  // os::naked_short_sleep(999);
  // Modified by Haoran
  while(heap->_mem_server_flags->_tracing_finished == false) {
    os::naked_short_sleep(10);
    syscall(RDMA_READ, 0, heap->_mem_server_flags, FLAGS_OF_MEM_SERVER_STATE_SIZE);
  }
  ...
  // Haoran: set ShenandoahRefProcFrequency to 0 disable this
  // If not cancelled, can try to concurrently pre-clean
  heap->entry_preclean();
  log_debug(semeru)("Memory Server Finishes PreClean!");
  ...
  if(!heap->collection_set()->is_empty()) {
    ...
    log_debug(semeru)("_evacuation_finished: %d", heap->_mem_server_flags->evacuation_finished);
