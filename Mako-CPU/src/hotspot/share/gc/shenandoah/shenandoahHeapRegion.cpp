...
ShenandoahHeapRegion::ShenandoahHeapRegion(ShenandoahHeap* heap, HeapWord* start,
  ...
  _evac_top(NULL),
  _selected_to(false)
  // , _live_data(0)  Modified by Haoran
{
  // Modified by Haoran
  // ContiguousSpace::initialize(_reserved, true, committed);
  initialize(_reserved, true, committed);
...
void ShenandoahHeapRegion::report_illegal_transition(const char *method) {
  ResourceMark rm;
  stringStream ss;
  ss.print("Illegal region 0x%lx state transition from \"%s\", at %s\n  ", region_number(), region_state_to_string(_state), method);
  print_on(&ss);
  fatal("%s", ss.as_string());
}

// Modified by Haoran for compaction
void ShenandoahHeapRegion::make_regular() {
  set_state(_regular);
}
...
void ShenandoahHeapRegion::recycle() {
  ...
  clear_live_data();
  _sync_between_mem_and_cpu->_top = bottom();
...
void ShenandoahHeapRegion::setup_sizes(size_t max_heap_size) {
  ...
  MaxTLABSizeWords = MIN2(ShenandoahElasticTLAB ? RegionSizeWords : (RegionSizeWords / 64), HumongousThresholdWords);
...
HeapWord* ShenandoahHeapRegion::get_tams() {
  return _heap->marking_context()->top_at_mark_start(this);
}
