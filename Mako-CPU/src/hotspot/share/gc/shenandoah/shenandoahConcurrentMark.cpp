...
private:
  ...
  inline void do_oop_work(T* p) {
    // Modified by Haoran
    // ShenandoahConcurrentMark::init_semeru_mark_through_ref<T, UPDATE_REFS, NO_DEDUP>(p, _heap, _queue, _mark_context);
    ShenandoahConcurrentMark::init_mark_through_ref<T, UPDATE_REFS, NO_DEDUP>(p, _heap, _queue, _mark_context, _worker_id);
    // ShenandoahConcurrentMark::satb_mark_through_ref<T, UPDATE_REFS, NO_DEDUP>(p, _heap, _queue, _mark_context);
    // ShenandoahConcurrentMark::mark_through_ref<T, UPDATE_REFS, NO_DEDUP>(p, _heap, _queue, _mark_context);
  }

public:
  ShenandoahInitMarkRootsClosure(ShenandoahObjToScanQueue* q, size_t worker_id) :
    _queue(q),
...
public:
  ...
  void work(uint worker_id) {
    ...
    ShenandoahInitMarkRootsClosure<UPDATE_REFS> mark_cl(q, worker_id);
...
void ShenandoahConcurrentMark::mark_roots(ShenandoahPhaseTimings::Phase root_phase) {
  ...
  // Haoran: TODO
  // Do not collect code cache
  // Haoran: use jvm flags to disable this
...
void ShenandoahConcurrentMark::mark_loop_prework(uint w, ShenandoahTaskTerminator *t, ReferenceProcessor *rp,
  ...
  if (_heap->unload_classes()) {
    ...
    if (_heap->has_forwarded_objects()) {
      ...
      if (strdedup) { // Haoran: dedup default false
        // Modified by Haoran
        ShouldNotReachHere();
...
// Haoran: obj in mark queue is always real refs.
