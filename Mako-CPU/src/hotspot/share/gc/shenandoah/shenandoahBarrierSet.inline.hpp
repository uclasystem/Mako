...
bool ShenandoahBarrierSet::arraycopy_element(T* cur_src, T* cur_dst, Klass* bound, Thread* const thread, ShenandoahMarkingContext* const ctx) {
  ...
  if (!CompressedOops::is_null(o)) {
    ...
    // Modified by Haoran for new
    assert(!_heap->in_evac_set(obj), "wrong invariant");
#ifdef RELEASE_CHECK
...
bool ShenandoahBarrierSet::AccessBarrier<decorators, BarrierSetT>::oop_arraycopy_in_heap(arrayOop src_obj, size_t src_offset_in_bytes, T* src_raw,
  ...
  if(heap->is_evacuation_in_progress()) {
    if(src_obj != NULL)
      ShenandoahBarrierSet::barrier_set()->load_reference_barrier_array_copy((oop*)src_obj);
    if(dst_obj != NULL)
      ShenandoahBarrierSet::barrier_set()->load_reference_barrier_array_copy((oop*)dst_obj);
  }
