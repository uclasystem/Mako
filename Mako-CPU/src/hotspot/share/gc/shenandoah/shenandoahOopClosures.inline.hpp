...
inline void ShenandoahMarkRefsSuperClosure::work(T *p) {
  // Modified by Haoran
  // ShouldNotReachHere();
  // ShenandoahConcurrentMark::mark_through_entry<T, UPDATE_REFS, STRING_DEDUP>(p, _heap, _queue, _mark_context);
  ShenandoahConcurrentMark::mark_through_ref<T, UPDATE_REFS, STRING_DEDUP>(p, _heap, _queue, _mark_context);
}

template <class T>
inline void ShenandoahVerifyHeapRefsClosure::do_oop_work(T* p) {
  _heap->verify_heap_ref(p);
}
...
// template <class T>
// inline void ShenandoahRootUpdateHeapRefsClosure::do_oop_work(T* p) {
//   _heap->semeru_root_update_with_forwarded(p);
// }

// template <class T>
// inline void ShenandoahSemeruUpdateHeapRefsClosure::do_oop_work(T* p) {
