...
public:
  ...
  // inline ShenandoahHeapRegion* const get_corr_region(size_t region_idx) const;
...
public:
  ...
  Monitor* _assure_read_monitor;
  char* _taken;

  void assure_read(size_t server_id);
  void assure_read_no_wait(size_t server_id);
  void assure_write(size_t server_id);

  void assure_read_for_region_index_no_sleep(size_t region_index);
...
private:
  ...
  void switch_offset_table(ShenandoahHeapRegion* region, ShenandoahHeapRegion* to_region);
  void op_final_sync(size_t max_region_num_for_record);
  void op_do_updaterefs();
...
private:
  ...
  RootObjectUpdateQueue*     _root_object_update_queue;
...
public:
  ...
  RootObjectUpdateQueue* root_object_update_queue() { return _root_object_update_queue; }
  size_t  get_server_id(HeapWord* addr);
  static void rdma_write_all(char* start_addr, size_t byt_size) {
    for(size_t i = 0; i < NUM_OF_MEMORY_SERVER; i++) {
      syscall(RDMA_WRITE, i, start_addr, byt_size );
    }
  }
  static void rdma_write(size_t server_id, char* start_addr, size_t byt_size) {
    syscall(RDMA_WRITE, server_id, start_addr, byt_size );
  }
  static void rdma_signal_flag(size_t server_id, char* start_addr, size_t byt_size) {
    syscall(RDMA_SIGNAL, server_id, start_addr, byt_size );
  }
  static void rdma_read(size_t server_id, char* start_addr, size_t byt_size) {
    syscall(RDMA_READ, server_id, start_addr, byt_size );
  }
  static void rdma_evict(size_t server_id, char* start_addr, size_t byt_size) {
    syscall(RDMA_EVICT, server_id, start_addr, byt_size );
  }


  // Haoran: size is in bytes
  static size_t cache_size_in_bytes(size_t region_index);
  // Haoran: size is in bytes
  static size_t cache_size_in_bytes(HeapWord* start_addr, size_t byt_size) {
    size_t num_pages = syscall(SYS_NUM_SWAP_OUT_PAGES, start_addr, byt_size);
    return byt_size - num_pages * 4096;
  }

  static size_t cache_ratio(size_t region_index);

  void rdma_read_bitmap_all();
  void rdma_read_bitmap(size_t server_id);

  void rdma_write_bitmap_all();
  void rdma_write_bitmap(size_t server_id);
  // void rdma_read_offset_table(size_t region_index);
  // void rdma_write_offset_table(size_t region_index);
  void rdma_read_alive_table(size_t server_id);
  void rdma_write_alive_table(size_t server_id);
  void push_to_root_object_update_queue(oop p, size_t worker_id);
  void update_root_objects();
...
public:
  ...
  bool supports_object_pinning() const { 
    // return false;
    return true; 
  }
...
public:
  ...
  HeapWord* allocate_memory_in_safepoint(ShenandoahAllocRequest& request);
  ...
  void flush_cache_for_region(ShenandoahHeapRegion *r);
...
public:
  ...
  static void check_range(HeapWord* addr) {

    assert((size_t)addr >= RDMA_DATA_SPACE_START_ADDR, "Wrong Invariant for addr!");
    assert((size_t)addr - RDMA_DATA_SPACE_START_ADDR <= 32*1024*1024*1024ULL, "Wrong Invariant for addr!");
  }
...
private:
  ...
  ShenandoahCollectionSet* _evac_set;
...
public:
  ...
  template <class T>
  bool in_evac_set(T addr) const;

  template <class T>
  bool atomic_in_evac_set(T addr) const;
...
public:
  ...
  template <class T>
  inline void verify_heap_ref(T* p);

  template <class T>
  inline void semeru_maybe_update_with_forwarded(T* p);

  // template <class T>
  // inline void semeru_root_update_with_forwarded(T* p);
