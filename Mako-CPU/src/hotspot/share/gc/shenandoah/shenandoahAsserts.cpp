...
void ShenandoahAsserts::assert_correct(void* interior_loc, oop obj, const char* file, int line) {
  ...
  // // Modified by Shi
  // assert(Universe::in_table(fwd), "fwd not in table in ShenandoahAsserts::assert_correct");
  
  // // Step 2. Check that the entry is from the correct table
  // size_t obj_region_idx = heap->heap_region_index_containing(obj);
  // obj_region_idx %= (heap->num_regions() / 2);
  // size_t entry_region_idx = Universe::table_index(fwd);

  // assert(obj_region_idx == entry_region_idx, "entry must be from the correct table");
...
void ShenandoahAsserts::assert_not_in_cset_loc(void* interior_loc, const char* file, int line) {
  ShenandoahHeap* heap = ShenandoahHeap::heap_no_check();
  if (heap->in_evac_set(interior_loc)) {
