...
// Modified by Shi
Mutex*   IndiTable_lock               = NULL;
// Mutex*   IndiTableFreeList_lock       = NULL;

// Modified by Haoran
Monitor* AssureRead_lock = NULL;      
#define MAX_NUM_MUTEX 128 + 3
...
void mutex_init() {
  ...
  if (UseShenandoahGC) {
    ...
    // Modified by Shi
    // Use native rank for now, as it escapes lock ranking and we do not think these two locks
    // will lead to deadlocking.
    // ref: 1. comment of assert check in Monitor::set_owner_implementation (around line 1300)
    // 2. deadlock and lock ordering section of http://web.mit.edu/6.031/www/sp20/classes/22-locks
    def(IndiTable_lock             , PaddedMutex  , native,      true,  Monitor::_safepoint_check_never);
    // def(IndiTableFreeList_lock     , PaddedMutex  , native,      true,  Monitor::_safepoint_check_never);
    // Modified by Haoran
    def(AssureRead_lock             , PaddedMonitor, access,      true,  Monitor::_safepoint_check_never);
